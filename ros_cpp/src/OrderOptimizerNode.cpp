...
OrderOptimizerNode::OrderOptimizerNode() : Node("OrderOptimizer")
{
  ...
  file = fs::absolute("/home/ubuntu/amr_example_small/amr_example");
  this->declare_parameter("path", file);
  sub1_ = this->create_subscription<geometry_msgs::msg::PoseStamped>("currentPosition", 10, std::bind(&OrderOptimizerNode::msgCurrentPos, this, std::placeholders::_1));
  sub2_ = this->create_subscription<msg_package::msg::Order>("nextOrder", 10, std::bind(&OrderOptimizerNode::msgNextOrder, this, std::placeholders::_1));
  ...
  for (auto &folder : std::filesystem::recursive_directory_iterator(path_)) {
    if (folder.is_directory() && (folder.path().filename().string()).compare("orders") == 0) {
      ...
      for (auto &file : std::filesystem::directory_iterator(folder.path())) {
        found = false;
        std::thread thread(&OrderOptimizerNode::parseOrderFile, this, file.path(), order_id, std::ref(found), &details_,
                           std::ref(details_.products));
...
std::vector<std::pair<float, Part>> OrderOptimizerNode::FindShortestPath(OrderDetails details_)
{
  // TODO: Implement me
//select all parts up
std::vector<Part>part_to_pick;
std::vector<std::pair<float, Part>> return_vec;
std::pair<float, Part> temp;

 for(auto it : details_.products) {
  for(auto it2 : products) {
    if(it2.second.product_name == it) //found this specific product 
    {
      for(auto it3 : it2.second.parts) {
        it3.distance = distance(current_pose->pose.position.x, it3.cx, current_pose->pose.position.y,it3.cy);
        part_to_pick.push_back(it3);
      }
    }
  }
 }
...
while(1) {
  ...
  int counter = 0;
  for(auto it : part_to_pick) {
    if(it.distance < min_distance) {
      min_distance = it.distance;
      index_minimum = counter;
    }  
    counter++;
  }
  temp.first = 1; //no idea
  temp.second = part_to_pick.at(index_minimum);
  return_vec.push_back(temp);
  //part_to_pick.erase(part_to_pick.begin()+ index_minimum); //removing first index
}
